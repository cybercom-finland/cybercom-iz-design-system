...
export function Table(props) {
  ...
  const getCellElement = (element, content, size) => {
    ...
    const getPriorityAndType =
      elementClassName === "type" || elementClassName === "priority" ? (
        <Typography
          type="body1"
          withIcon
          iconName={content ? content.toLowerCase() : "incident"}
          text={content}
        />
      ) : (
        ""
    ...
    component = (
      <ElementSize key={element} className={elementClassName}>
        <div>
          {getPriorityAndType}
          <span>{getRowContent}</span>
        </div>
      </ElementSize>
    );
    return component;
  ...
  const providedTableHeaders =
    props.headers !== undefined ? props.headers : Object.keys(props.data[0]);
  ...
  const getSortingDirectionIcon = (header) => {
    if (!sortingItemConfig) {
      return (
        <>
          <Icon className="ascending" icon="arrowsmallup" />
          <Icon className="descending" icon="arrowsmalldown" />
        </>
      );
    }
    return sortingItemConfig.key === header &&
      sortingItemConfig.direction === "descending" ? (
      <Icon className="descending" icon="arrowsmalldown" />
    ) : sortingItemConfig.key === header &&
      sortingItemConfig.direction === "ascending" ? (
      <Icon className="ascending" icon="arrowsmallup" />
    ) : (
      <>
    ...
    <StyledPagination className="pagination">
      ...
      <div className="items-information">
        {`${
          currentPageValue === 1
            ? currentPageValue
            : rowsPerPageValue * currentPageValue - rowsPerPageValue + 1
        } - ${lastRowIndex <= items.length ? lastRowIndex : items.length} of ${
          items.length
        } items`}
      </div>
      ...
      <div className="arrows">
        <Icon
          icon="arrowleft"
          className={currentPageValue === 1 ? "disabled" : null}
          onClick={() => setTablePage("back")}
        />
        <hr />
        <Icon
          icon="arrowright"
          className={
            currentPageValue === pageNumbers.length ? "disabled" : null
          }
          onClick={() => setTablePage("forward")}
        />
      </div>
    </StyledPagination>
  );
                      ...
                      <div className="sorting-header">
                        {header.charAt(0).toUpperCase() + header.slice(1)}{" "}
                        {
                          <div className="sorting-direction-arrow-container">
                            {getSortingDirectionIcon(
                              `${providedDataKeys[index]}`
                            )}
                          </div>
                        }
                      </div>
